#!/usr/bin/env Rscript
# https://github.com/hukai916/scATACpipe/blob/main/bin/correct_barcode.R
...
option_list = list(
	...
	make_option(c("--assay_type"), type="character", default=NULL,
							help="assay type: ATAC / mmATAC / mmRNA", metavar="character"))

opt_parser = OptionParser(option_list = option_list);
opt = parse_args(opt_parser);

if (is.null(opt$barcode_file) | is.null(opt$assay_type)) {
	print_help(opt_parser)
	stop("At least the barcode_file AND ASSAY TYPE must be provided!")
}
...
correct_barcode <- function(barcode_file, reads_per_chunk, path_output_fq, assay_type) {
    ...
    if (assay_type == "mmRNA") {
  		whitelist_file <- "/media/leon/Polina/atac_rna/cellranger_arc/cellranger-arc-2.0.2/lib/python/cellranger/barcodes/737K-arc-v1.txt"
	} else if (assay_type == "mmATAC") {
  		# whitelist_file <- "/media/leon/Polina/atac_rna/cellranger_arc/cellranger-arc-2.0.2/lib/python/atac/barcodes/737K-arc-v1.txt"
   		whitelist_file <- "/media/leon/Polina/atac_rna/cellranger_arc/cellranger-arc-2.0.2/lib/python/atac/barcodes/RC_737K-arc-v1.txt"
	} else if (assay_type == "ATAC") {
        whitelist_file <- "/media/leon/Masha/ATAC/cellranger-atac-2.1.0/lib/python/atac/barcodes/737K-cratac-v1.txt"
    }
	...
	if (assay_type == "mmRNA") {
		...
		whitelist_file <- "/media/leon/Masha/ATAC/cellranger-atac-2.1.0/lib/python/atac/barcodes/737K-cratac-v1.txt"
		if (any(c("SRR14048778", "SRR14048779", "SRR14048783") %in% barcode_file)) {
        	whitelist_file <- "/media/leon/Masha/ATAC/cellranger-atac-2.1.0/lib/python/atac/barcodes/RC_737K-cratac-v1.txt"
    	}	
	}
	...
	while (length(fq <- yield(f))) {
		...
		if (assay_type == "mmRNA") {
  			reads <- substr(reads, 1, 16)
		} else if (assay_type == "mmATAC") {
  			reads <- substr(reads, 9, 24)
		}
	...
	# Создаётся временный tagfile, который затем переименовывается
	temp_tagfile <- paste0(path_output_fq, "/tem_tagfile_", basename(barcode_file), ".txt")
	if (file.exists(temp_tagfile)) file.remove(temp_tagfile)
	...
	while (length(fq <- yield(f))) {
    	...
    	write(taglines, temp_tagfile, append = TRUE)
	...
	# Переименовываем временный tagfile в финальный
	final_tagfile <- paste0(path_output_fq, "/tagfile_", basename(barcode_file), ".txt")
	if (file.exists(final_tagfile)) file.remove(final_tagfile)
	file.rename(temp_tagfile, final_tagfile)
...
correct_barcode(barcode_file = opt$barcode_file, reads_per_chunk = opt$reads_per_chunk, path_output_fq = opt$path_output_fq, assay_type = opt$assay_type)
